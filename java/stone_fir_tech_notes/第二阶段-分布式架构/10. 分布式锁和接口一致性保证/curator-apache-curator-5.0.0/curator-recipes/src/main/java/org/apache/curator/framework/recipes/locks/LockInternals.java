...
public class LockInternals
{
    ...
    private final String                            path;     // "/xxxx/xxxx/locks/lock_01/lock-"
    private final String                            basePath; // "/xxxx/xxxx/locks/lock_01"
    private final LockInternalsDriver               driver;
    private final String                            lockName; // "lock-"
    private final AtomicReference<RevocationSpec>   revocable = new AtomicReference<RevocationSpec>(null); // 这个应该是设置一些可以取消锁的一些操作.
    ...
    private volatile int    maxLeases;  // 可重入次数, 第一次就是1.
    ...
    LockInternals(CuratorFramework client, LockInternalsDriver driver, String path, String lockName, int maxLeases)
    {
        ...
        this.basePath = PathUtils.validatePath(path); // "/xxxx/xxxx/locks/lock_01"
        this.path = ZKPaths.makePath(path, lockName); // "/xxxx/xxxx/locks/lock_01/lock-"
    }
    ...
    // TODO: A.1.1: 这个是具体的zk加锁.
    String attemptLock(long time, TimeUnit unit,
                       byte[] lockNodeBytes //默认为null
    ) throws Exception
    {
        final long      startMillis = System.currentTimeMillis();
        final Long      millisToWait = (unit != null) ? unit.toMillis(time) : null;
        final byte[]    localLockNodeBytes = (revocable.get() != null) ? new byte[0] : lockNodeBytes; // null
        int             retryCount = 0;
        ...
        while ( !isDone )
        {
            ...
            try
            {
                // 这里创建一个临时节点: {{bathPath}}/{{GUID}}lock-{{sequence}} 这里几乎一定能加成功, 因为临时顺序节点嘛, 往下顺延就好了
                ourPath = driver.createsTheLock(client, path, localLockNodeBytes);
                // 这里是检查自己是不是第一个顺序节点, 不是的话就阻塞等着.
    ...
    private boolean internalLockLoop(long startMillis, Long millisToWait, String ourPath) throws Exception
    {
        ...
        try
        {
            ...
            // 这里是重新拿了一下这个锁, 如果拿到了就返回true.
            while ( (client.getState() == CuratorFrameworkState.STARTED) && !haveTheLock )
            {
                // 把basePath下面所有的临时有序node都拿出来
                List<String>        children = getSortedChildren();
                // 取出拿到的锁地址的锁name, 如果没拿到就会报错.
                String              sequenceNodeName = ourPath.substring(basePath.length() + 1); // +1 to include the slash

                // TODO: *看看有没有拿到锁: 检查当前node的index有没有在可重入次数之内, 如果没有说明没拿到(监听前一个锁地址), 如果在说明拿到了.
