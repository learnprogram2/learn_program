...
// 脚本
----------- 设置总信号量
key: [keyName, channelName]
args: [permits(总的信号量)]
// 找到keyName对应的信号量总量
local value = redis.call('get', KEYS[1]);
if (value == false or value == 0) then
// 把总信号量设置进去, 然后发布一条消息.说自己的总信号量
redis.call('set', KEYS[1], ARGV[1]);
redis.call('publish', KEYS[2], ARGV[1]);
return 1;
end;
// 之前设置过或者已经没有信号量了, 就返回0.
return 0;


-------- 请求信号量
key:[keyName]
argus:[permits(要求的信号量)]
// 拿到当前的总信号量
local value = redis.call('get', KEYS[1]);
// 信号量够, 就直接减, 返回1.
...
----------- 释放信号量
key: [keyName, channelName(redisson_sc:{keyName})]
args: [permits(释放的信号量)]
// 直接尝试增加keyName的信号量, 然后广播一条消息
...
public class RedissonSemaphore extends RedissonExpirable implements RSemaphore {
    ...
    // TODO: G.1: 这里是拿信号量的地方, acquire(), 默认拿一个信号量.
    @Override
    ...
    public void acquire(int permits) throws InterruptedException {
        ...
        // 拿不到, 就订阅一个channel, 然后一直尝试acquire()
...
//        get(acquireAsync(permits));
    ...
    public boolean tryAcquire(int permits) {
        return // 同步拿到future的结果
                get(
                        // 异步拿信号
                        tryAcquireAsync(permits));
    }
